...
async def tanlash(call: types.CallbackQuery, state: FSMContext):
    ...
    if r == "ha":
        ...
        await call.message.answer(
            f"<b><i>❌E`lon kanalga joylanmadi\nAvtomobilingizni sotmoqchi bo'lsangiz <u>E'lon berish</u> tugmasi bosing!</i></b>",
            parse_mode="html", reply_markup=kirish)
        await state.finish()
...
async def tekshirish(call: types.CallbackQuery):
    ...
    if final_status:
        ...
        await holat.kir.set()
        await call.message.answer(
            f"<b><i>Avtomobilingizni sotmoqchi bo'lsangiz <u>E'lon berish</u> tugmasi bosing! </i></b>",
            parse_mode="html",
            reply_markup=kirish)
    else:
        await call.message.delete()
